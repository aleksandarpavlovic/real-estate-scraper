...
public class NekretnineRsCriteriaTransformer {
    ...
    private void transformMultivalueCriteria(MultivalueCriteria criteria, NekretnineRsRequest request) {
        ...
        if (ADVERTISER.equals(criteriaName)) {
            ...
            NekretnineRsRequest.Path heatingTypePath = new NekretnineRsRequest.Path("vrsta-grejanja", concatenate(getMappings(listIntersection(criteria.getValues(), Arrays.asList(HeatingType.CENTRAL, HeatingType.OWN_CENTRAL)))));
            NekretnineRsRequest.Path fuelTypePath = new NekretnineRsRequest.Path("vrsta-goriva", concatenate(getMappings(listIntersection(criteria.getValues(), Arrays.asList(HeatingType.ELECTRIC, HeatingType.GAS)))));
